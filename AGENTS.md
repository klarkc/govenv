...
Pull requests use a soft learning gate. Every substantive candidate must refresh the explicit candidate learning assessment against the exact PR base: classify the candidate kind and learning impact, explain the judgment in fresh rationale, and list every new learning requirement. Classification quality remains Protocol judgment reviewed by the human in the PR; do not infer it from Conventional Commit labels or let an authoring agent's assertion count as learning evidence. If governed learning debt is empty, non-expanding work may proceed normally; concept-expanding feature/refactor work may merge only after every candidate requirement has human-produced evidence and all debt is closed. Urgent corrective work may bypass unresolved learning only through the explicit corrective bypass and must preserve every unsatisfied candidate requirement as outstanding debt; bypass never closes, rewrites, or hides debt. Candidate Test CI applies the governed gate and assessment freshness at the direct pull-request boundary. Patch releases may carry known debt, while major and minor releases require zero outstanding learning debt.
