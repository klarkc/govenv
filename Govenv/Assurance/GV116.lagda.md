...
derived from that history, a validated one-time genesis boundary for legacy
cutover, and typed revision-bound snapshots that preserve an exact append-only
history prefix. It does **not** mark GV116 complete; Govenv's real genesis, the
live roadmap/materialized snapshot, and release deltas still remain to migrate
onto the constitutional prefix.
